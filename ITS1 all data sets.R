...
### OBJECTIVE: to perform quality control and preliminary analysis for all
###            ITS1 data sets simultaneously - this will result in rarefying
###            all data sets to the same depth
...
# see this site for a good walkthrough of microbiome analysis:
# https://www.nicholas-ollberding.com/post/introduction-to-the-statistical-analysis-of-microbiome-data-in-r/


# read in QIIME2 files:
...
# library(decontam)
# # this workflow copied and pasted from:
# #    https://benjjneb.github.io/decontam/vignettes/decontam_intro.html
# 
# # inspect library sizes for samples and negative controls:
# df <- as.data.frame(sample_data(pseq_alpha)) # Put sample_data into a ggplot-friendly data.frame
# df$LibrarySize <- sample_sums(pseq_alpha)
# df <- df[order(df$LibrarySize),]
# df$Index <- seq(nrow(df))
# ggplot(data=df, aes(x=Index, y=LibrarySize, color=Sample_or_Control)) + geom_point()
# 
# # ok, so the negative controls have a fair amount of reads compared to the positive controls...
# 
# # identify contaminants with the prevalence method - this does not require DNA concentrations:
# sample_data(pseq_alpha)$is.neg <- sample_data(pseq_alpha)$PosNeg == "Neg"
# contamdf.prev <- isContaminant(pseq_alpha, method="prevalence", neg="is.neg")
...
# #install_version("BiocManager", version = "1.30.9", repos = "http://cran.us.r-project.org")
# ps.pa <- transform_sample_counts(pseq_alpha, function(abund) 1*(abund>0))
...
# 
# # ok, I think this looks mostly good; now we can remove these contaminants from the phyloseq
# #   object:
# ps.noncontam <- prune_taxa(!contamdf.prev$contaminant, pseq_alpha)
# # compare the number of taxa from before decontamination:
# pseq_alpha
# # to those after decontamination:
# ps.noncontam
# 
# burk2019_decontam <- subset_samples(ps.noncontam, Year == 2019 & Sampler_Type == "Burkard")
# roto_decontam <- subset_samples(ps.noncontam, Year == 2019 & Sampler_Type == "Rotorod")
# burk2021_decontam <- subset_samples(ps.noncontam, Year == 2021 & Sampler_Type == "Burkard")
# 
# saveRDS(ps.noncontam, "Phyloseq objects/its1_all_decontam.rds")
# saveRDS(burk2019_decontam, "Phyloseq objects/burk2019_its1_decontam.rds")
# saveRDS(roto_decontam, "Phyloseq objects/roto_its1_decontam.rds")
# saveRDS(burk2021_decontam, "Phyloseq objects/burk2021_its1_decontam.rds")


# NB: decontaminating the water sample dramatically increases the alpha diversity of the 
#     water sample - this is because it removes reads from an ASV that comprised the vast majority
#     of reads in the whole sample... Hmm, must think about this a bit more

# ALPHA DIVERSITY ANALYSIS ------------------------------------------------

# RAREFACTION:
# make rarefaction curves:
# NB: this command takes a while to run... maybe there are parameters I can specify to speed it up
# .... ok this actually takes FAR too long. Must find a faster way to do this
# rarecurve(t(otu_table(pseq_alpha)), size = 1000, step = 200)
# rarefy all to lowest read depth:
# pseq_alpha <- ps.noncontam
sample_counts <- data.frame(sample_sums(pseq_alpha))
sample_counts <- rownames_to_column(sample_counts)
sample_counts[which.min(sample_counts$sample_sums.pseq_alpha.),]

to_keep <- filter(sample_counts, sample_sums.pseq_alpha. > 15000)$rowname

# rarefaction curve:
#rarecurve(t(otu_table(pseq_alpha)), step=1000, cex=0.5, 
#          label = F, xlim = c(0, 20000))
# ok, so the sample with the lowest read count is H2OBlank03_ITS1 with a read count of
#   60321; try removing this sample and see what proceed with rarefaction 
#   and see what happens ...
pseq_alpha_filt <- subset_samples(pseq_alpha, SampleID %in% to_keep)

# NB this function rarefies to the depth of the lowest sample; samples WITH
#   replacement
pseq_rare <- rarefy_even_depth(pseq_alpha_filt, rngseed = 314)
# 7310 OTUs removed because they were no longer present after subsampling

burk2019_rare <- subset_samples(pseq_rare, Year == 2019 & Sampler_Type == "Burkard")
roto_rare <- subset_samples(pseq_rare, Year == 2019 & Sampler_Type == "Rotorod")
burk2021_rare <- subset_samples(pseq_rare, Year == 2021 & Sampler_Type == "Burkard")

saveRDS(pseq_rare, "Phyloseq objects/its1_all_rare.rds")
saveRDS(burk2019_rare, "Phyloseq objects/burk2019_its1_rare.rds")
saveRDS(roto_rare, "Phyloseq objects/roto_its1_rare.rds")
saveRDS(burk2021_rare, "Phyloseq objects/burk2021_its1_rare.rds")
